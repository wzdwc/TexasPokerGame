...
describe("test/app/core/pokerStyle.test.ts", () => {
  it("Royal Flush", async () => {
    let pokerStyle: PokerStyle = new PokerStyle([
      "i1",
      "j1",
      "k1",
      "l1",
  ...
  it("compare card", async () => {
    let pokerStyle: PokerStyle = new PokerStyle(
      ["k2", "k3", "g2", "e4", "m3", "m1", "g1"],
      true
    );
    let pokerStyle2: PokerStyle = new PokerStyle(
      ["k2", "k3", "g2", "e4", "m3", "m2", "c2"],
      true
    );
    console.log("getPokerValueCard", pokerStyle.getPokerValueCard());
    assert.strictEqual(
      pokerStyle.getPokerWeight(),
      pokerStyle2.getPokerWeight()
    );
  });
});
